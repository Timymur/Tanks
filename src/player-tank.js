import { Keys, PLAYER1_TANK_POSITION, PLAYER1_TANK_SPRITES, TANK_SPEED } from './constans.js';
...
export default class PlayerTank extends Tank {
    ...
    update({ input, frameDelta, stage }) {
        if (input.has(Keys.UP, Keys.RIGHT, Keys.DOWN, Keys.LEFT)) {
            ...
            this._turn(direction); // Методы в tank
        ...
        if (input.keys.has(Keys.SPACE)) {
            this._fire(); // Методы в tank

            if (this.bullet) {
                stage.objects.add(this.bullet);
            }
        }
    }
}